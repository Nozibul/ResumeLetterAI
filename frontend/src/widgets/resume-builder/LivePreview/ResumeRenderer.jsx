 ...
 * - Section-based layout with custom ordering
...
import { useSectionOrder } from '@/shared/store/hooks/useResume';
...
function ResumeRenderer({ resumeData, templateId }) {
  ...
  // Get section order from Redux
  // ✅ Get section order with safe default
  const sectionOrderFromRedux = useSectionOrder();
  ...
  const sectionOrder = useMemo(() => {
    ...
    if (
      Array.isArray(sectionOrderFromRedux) &&
      sectionOrderFromRedux.length > 0
    ) {
      return sectionOrderFromRedux;
    }
    return defaultOrder;
  }, [sectionOrderFromRedux]);
  ...
  const sectionComponents = useMemo(
    () => ({
      ...
      competitiveProgramming: () =>
        ...
        validData.competitiveProgramming?.length > 0 && (
          <section className="mb-4">
            <h2 className="text-md font-semibold text-gray-900 uppercase mb-2 border-b border-gray-300 pb-1">
              Competitive Programming
            </h2>
            <div className="space-y-1 text-xs">
              {validData.competitiveProgramming.map((cp, index) => (
                <div key={cp._id || index}>
                  <strong>{cp.platform}:</strong> {cp.problemsSolved} problems
                  solved
                </div>
              ))}
            </div>
    ...
    [validData, isSectionVisible, sanitizeText]
  );
