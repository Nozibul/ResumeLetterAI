...
const ResumeCategoryCard = memo(
  ({ template, isActive = true, onClick, className = '' }) => {
    ...
    return (
      <div
        className={`
          w-48 h-64 bg-white rounded-xl shadow-lg border-2 p-4 transition-all duration-800 cursor-pointer relative transform
          ${
            isActive
              ? `${borderColors[color]} shadow-2xl scale-110 z-10`
              : 'border-gray-200 hover:border-blue-200 hover:shadow-xl hover:scale-102'
          }
          hover:rotate-1 ${className}
        `}
        onClick={onClick}
        role="button"
        tabIndex={0}
      ...
      >
        {/* Template Header */}
        <div
          className={`h-8 bg-gradient-to-r ${colorGradients[color]} rounded-lg mb-3 flex items-center px-3 transition-all duration-300`}
        >
        ...
        {isActive && (
          <>
            ...
            <div className="absolute top-1/2 -right-3 transform -translate-y-1/2 w-8 h-8 bg-white rounded-full shadow-xl flex items-center justify-center animate-spin 100s liniar infinite">
              <Icon iconName="zap" size="sm" className={`text-${color}-500`} />
            </div>
            ...
            <div className="absolute top-1/2 -left-3 transform -translate-y-1/2 w-8 h-8 bg-white rounded-full shadow-xl flex items-center justify-center animate-spin 100s liniar infinite">
              <Icon iconName="zap" size="sm" className={`text-${color}-500`} />
            </div>
          </>
        )}
